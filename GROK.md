...
src/
  ...
  main/
    ...
    agent/
      ...
      shell.ts          child-process runner for bash/monitor/diagnostics (process groups, no pipe deadlock)
...
- **Shelling out:** every command-running tool goes through `agent/shell.ts` — never `spawn` + `'close'` directly. Two traps it exists to avoid: `'close'` waits for all inherited pipes to close, so a command that leaves a process behind (`npm run dev &`) never fires it and hangs the turn; and `child.kill()` signals only the shell, so grandchildren survive a timeout. It settles on `'exit'` with a bounded drain, and spawns `detached` so kills hit the whole process group.
